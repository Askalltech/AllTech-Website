 ...
 *
 * Adapted from a community component: dropped its hardcoded Geist @font-face
 * injection (it forced a typeface unrelated to this site's own type system —
 * the carousel just inherits whatever font its container sets) and swapped
 * `bg-muted` for a token this project actually registers (see global.css's
 * note on why `--color-muted` itself is deliberately unregistered here).
 */
                    ...
                    {cards.map((card, place) => {
                        ...
                        return (
                            <button
                                ...
                                className={cn(
                                    "relative isolate h-full shrink-0 cursor-pointer overflow-hidden p-0",
